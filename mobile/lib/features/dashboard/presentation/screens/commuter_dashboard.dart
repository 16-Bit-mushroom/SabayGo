...
class _CommuterDashboardState extends State<CommuterDashboard> {
  ...
  Widget _buildBody(BuildContext context, BookingViewModel viewModel) {
    ...
    if (_currentIndex == 1) return const Center(child: Text("Trip History UI"));
    if (_currentIndex == 2) return const Center(child: Text("Messages UI"));
    if (_currentIndex == 3) return const Center(child: Text("Profile UI"));
    ...
    switch (viewModel.currentStep) {
      ...
      case BookingStep.selectingPayment:
        return PaymentScreen(
          ...
          onPaymentMethodSelected: (String methodId) => viewModel.selectPaymentMethod(methodId),
                ...
                return AlertDialog(
                  ...
                  actions: [
                    TextButton(
                      ...
                      child: const Text("Keep Ride", style: TextStyle(color: AppColors.textSecondary)),
                    ),
                    ElevatedButton(
                      onPressed: () {
      ...
      case BookingStep.completed:
        return TripCompletedScreen(
          onReturnHome: () => viewModel.resetToHome(),
        );
